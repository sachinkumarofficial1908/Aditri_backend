...
const templatePath = path.join(
  __dirname,
  "../templates/wage-slip-template.xlsx"
);
...
const optionalHeaders = ["Other Allowance"];
...
function copyTemplateBlock(source, target, startRow) {
  ...
  // FIXED MERGES
  const merges = Array.isArray(source.model.merges)
    ? source.model.merges
    : Object.keys(source.model.merges || {});
...
function writeSlip(sheet, worker, month, year, startRow) {
  ...
  C("A2").value = company;
  C("D2").value = company;
  C("I2").value = `Month : ${String(month).toUpperCase()}-${String(
    year
  ).slice(-2)}`;
  ...
  // PF ON BASE
  C("J5").value = {
    formula: `ROUND(E${R(5)}*12%,2)`,
    result: c.pf,
  };

  // ESIC ON BASE
  C("J6").value = {
    formula: `ROUND(E${R(5)}*0.75%,2)`,
    result: c.esic,
  };
...
async function createWageSlipWorkbook({
  ...
  try {
    ...
    const { sheet, headerMap, missingHeaders } =
      await readInputSheet(inputPath);
    ...
    const { workers, skippedRows } = extractWorkers(
      sheet,
      headerMap
    );
    ...
    const outputSheet =
      outputWorkbook.addWorksheet("Wage Slips");
    ...
    workers.forEach((worker, index) => {
      const startRow =
        index * (templateRowCount + gapRows) + 1;
    ...
    console.error("WAGE SLIP ERROR:", err);
